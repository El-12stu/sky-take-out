...
public class OrderServiceImpl implements OrderService {
     ...
     * @param address
     */
    private void checkOutOfRange(String address) throws JSONException {
        Map map = new HashMap();
        ...
        //数据解析
        // 获取 result 中的 routes 数组
        Integer distance;
        com.alibaba.fastjson.JSONObject result = jsonObject.getJSONObject("result");
        JSONArray jsonArray = result.getJSONArray("routes");

        // 确保 jsonArray 不为空并且至少包含一个元素
        if (jsonArray != null && !jsonArray.isEmpty()) {
            // 获取第一个 route 对象
            com.alibaba.fastjson.JSONObject route = jsonArray.getJSONObject(0);

            // 使用 getInteger() 方法直接获取 distance，避免类型转换异常
            distance = route.getInteger("distance");

            if (distance != null) {
                System.out.println("配送距离: " + distance + " 米");
            } else {
                throw new OrderBusinessException("未能获取有效的距离数据");
            }
        } else {
            throw new OrderBusinessException("无效的路线数据");
        }
    ...
    public OrderPaymentVO payment(OrdersPaymentDTO ordersPaymentDTO) throws Exception {
       ...
       /* if (jsonObject.getString("code") != null && jsonObject.getString("code").equals("ORDERPAID")) {
        ...
        OrderPaymentVO vo = objectMapper.readValue(jsonObject.toString(), OrderPaymentVO.class);

       // OrderPaymentVO vo = jsonObject.optJSONObject(String.valueOf(OrderPaymentVO.class));
    ...
    /**
     *公共--根据Page<Orders> 返回List<OrderVO>
     * private公共方法
     * @param page
     * @return
     */
     ...
     * @throws Exception
     */
    public void userCancelByOrderId(Long orderId) throws Exception{
    ...
    public void reminder(Long id){
        ...
        map.put("content","订单号："+orderDB.getNumber());//获取订单号
